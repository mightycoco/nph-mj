        ...
        for(;;)
        {
                ...
                */
                while(n > 0)
                {
                        ...
                        if (nsend == -1)
                        {
                                syslog (LOG_INFO, "couldn't send data");
                                if (errno == EINTR)
                                        continue;
                                syslog (LOG_INFO, "client hard disconnected");
                                close(sockfd);
                                exit(1);
                        }
                        /* strange: stdout is close?! */
                        else if (nsend <= 0) {
                                syslog (LOG_INFO, "client disconnected - stdout is closed");
                                close(sockfd);
                                exit(1);
                        }
                ...
                if (difftime(time(NULL), begin) >= 50.0) {
                        syslog (LOG_INFO, "client connected since 50 seconds. force closing connection");
                        close(sockfd);
                        exit(1);
                }
        }
